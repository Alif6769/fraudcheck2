...
export default function ProductList() {
  ...
  const renderRootProduct = (product) => {
    ...
    return <Link to={`/app/inventory/product-mapping/${root.id}`}>{root.productName}</Link>;
  };
    ...
    return (
      <IndexTable.Row id={id} key={id} selected={selectedResources.includes(id)} position={index}>
        ...
        <IndexTable.Cell>
          ...
          <Link to={`/app/inventory/product-mapping/${id}`}>
  ...
  return (
    <Page
      title="Products"
      primaryAction={{ content: 'Add product', url: '/app/inventory/product-mapping/new' }}
    >
      <Card padding="0">
        <IndexTable
